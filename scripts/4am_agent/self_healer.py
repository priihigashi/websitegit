...
def _create_pr(module_name, fix, error):
    ...
    try:
        main_sha = requests.get(f"{base}/git/ref/heads/main", headers=_gh_headers(), timeout=15).json()["object"]["sha"]
        fp       = fix["file_to_edit"]
        fr       = requests.get(f"{base}/contents/{fp}", headers=_gh_headers(), timeout=15).json()
